...
**HWPX 빈 양식 파일은 두지 않는다.** 조립은 kordoc 계획서 프리셋으로 하고 PDF 규칙(A4 / 돋움 11pt / 160%)을 헤더 패치로 적용한다 — 절차는 `default_form_rules.md` §3, 명령 전문은 `hwpx-writing` 스킬 §3.

## 양식 출처·권리

원본은 **과기정통부-연구재단 연구개발계획서와 산업부-산기평 사업계획서의 공통(교집합) 항목만 추출해 경량화**한 범용 골격이다.
정부 표준 서식에서 파생한 **양식(빈 서식) 구조**만 담고 있으며, 특정 기관의 작성 내용·로고·식별정보는 포함하지 않는다.
도입 시 PDF 메타데이터(`title`·`creator`·`producer`)를 제거했고, 본문·메타데이터 전수 검사에서 기관·개인 식별정보 **0건**을 확인했다.
**실제 제출 시에는 그 공고의 첨부 서식이 항상 우선**하므로, 이 파일은 서식이 없을 때의 폴백 골격으로만 쓴다.
