...
1. **main 직접 push 금지.** 모든 변경은 브랜치 → PR → 리뷰 승인 → squash 머지. (저장소 관리자만 예외 — §7.1)
...
main 은 룰셋(`main-protect`)으로 보호된다 — PR 필수 · 승인 1명 · squash 전용 · 선형 이력 · force push/삭제 차단.
**단, `Repository admin` 역할은 이 룰셋을 우회한다.**

| | 역할 | main 직접 push | PR |
|---|---|---|---|
| 저장소 소유자 | admin | 가능 | 권장(의무 아님) |
| 그 외 기여자 | write | **불가** | **필수 · 승인 1명** |

관리자도 **다음 경우에는 PR 을 쓴다** — 다른 사람이 소유한 플러그인을 건드릴 때,
저장소 전역 파일(`marketplace.json`·`.gitignore`·이 문서)을 바꿀 때, 되돌리기 어려운 변경일 때.
자기 플러그인 안에서 끝나는 변경만 직접 push 한다.

관리자가 직접 push 할 때도 §3 의 규칙(경로 명시 커밋, 1커밋 1주제)은 그대로 지킨다.
리뷰를 건너뛴다는 것이지 규칙을 건너뛴다는 뜻이 아니다.
